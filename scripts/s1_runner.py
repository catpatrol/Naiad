"""S-1 — instrumented replay of the 20-cell grid, run TWICE (contract §5).

Engine 1.0.9, config v12_anchor_g8 (UNCHANGED), TC-4 windows verbatim.
Roots:
    research_outputs/s1/journal_s1/scored/<cell>/       + s1_events/
    research_outputs/s1/journal_s1_run2/scored/<cell>/  + s1_events_run2/
...
(the idempotent merge must never mask non-determinism).
    ...
    .venv/Scripts/python.exe scripts/s1_runner.py [--workers N]
"""
import argparse
import hashlib
import json
...
def one_cell(journal_root: str, sidecar_root: str, cell_id: str) -> dict:
    ...
    s = run_replay(CONFIG, cell_id, CELLS[cell_id], END, Path(journal_root),
                   ...
                   s1_sidecar_root=Path(sidecar_root),
                   s1_resampled_dir=RESAMPLED)
    s["cell_id"] = cell_id
...
def main() -> int:
    ...
    args = ap.parse_args()
    p1 = run_pass("journal_s1", "s1_events", args.workers)
    p2 = run_pass("journal_s1_run2", "s1_events_run2", args.workers)
    ...
    manifest = {
        "phase": "S-1 instrumented replay (measure-only)",
        "contract": "S1_Instrumented_Replay_Builder_Contract.md",
        "engine_version": ENGINE_VERSION,
        "config_id": CONFIG,
        "config_sha256": hashlib.sha256(
            (ROOT / "configs" / f"{CONFIG}.yaml").read_bytes()).hexdigest(),
        "baseline": "research_outputs/tc4/journal_pass2 (engine 1.0.8)",
