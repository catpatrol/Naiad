"""SS12-RANGEFINDER · THE MACHINE — the pure range lifecycle, importable.
...
sizing computation may read a range. Concretely: engine.signals,
engine.trading, engine.htf, engine.replay, posture_engine and every
tierc*_rules module stay strangers to this file; the range object exists in
the render and in the tape, nowhere else (F-BR-14 scans imports
component-wise for exactly that, and a planted gate read must go red).

LIFTED from scripts/rangefinder_twin.py at OR-1 (STEP D, 2026-09-21) WITH
EVENT LOGS BYTE-IDENTICAL [gate G-5]. The contract's words: "lift the
RangeFinder twin into engine/rangefinder.py as an importable module (same v2
pins, same event log; the twin becomes a thin caller; determinism fixture
F-RF-1 must still pass byte-identical)". What moved, VERBATIM: ATR_LEN, PINS,
V2_WINDOW_BARS, MEM_CAP_PER_SIDE, PINS_V2, Range, run_machine, _span,
_span_at, containment, flips_and_leash, run_v2, and the pinned readings that
are their law. What stayed in the twin: the cache loaders, the KEY-A/B/C
tables, scoring, calibration, the exports of record and the CLI; the twin
re-exports every name the fixtures and the pine-sim read.

THE ONE EDIT INSIDE THE MOVED CODE [TRAP 1]: flips_and_leash read the module
global MEM_CAP_PER_SIDE; it now takes `mem_cap`, a keyword that defaults to
this module's constant READ AT CALL TIME, and run_v2 forwards it. F-RF-10's
break leg monkeypatches rangefinder_twin.MEM_CAP_PER_SIDE — a plain re-export
would have made that patch inert and the fixture void. At the default the
bytes are the same, proven rather than argued.

THE BASELINE OF RECORD — sha256 of json.dumps(events, sort_keys=True) on the
FROZEN tape (BTCUSDT 4h truncated in memory to open_time <= 1787371200000,
2026-08-22T04:00Z), measured before the lift and re-measured equal after it:
...
The quirks ARE the bytes and none may be tidied: log() rounds every int or
float keyword to 2 dp THROUGH float() — so integer-valued fields ship as
floats ("p0_bar": 3.0, "closes": 8.0) — except rid/bars/count; event keys keep
insertion order (the exports do not sort); pivots are logged first and the
list is then STABLE-sorted on "i"; Range field order is the asdict() export
order; PINS / PINS_V2 key order is export order; pending/top carry np.float64;
ATR is engine.indicators.atr at ATR_LEN = 14, seeded at the window's first
bar; run_v2's "2026-02-01" is a BTC KEY-C literal and stays a literal.

NO ENGINE VERSION BUMP, by design: no trading behaviour changes, nothing in
the replay closure imports this file, and ENGINE_VERSION feeds every run_id —
a bump would rewrite every journal's bytes for a display organ.

IMPORT SURFACE, fixtured: numpy, pandas, dataclasses, engine.indicators.
Nothing else — no IO, no network, no wall clock, no cache path. The tierc2/3/4
fixtures raw-text-scan every engine/*.py (comments and docstrings count) for
an import of the outcome-statistics package; this file must never spell one.

ADDITIVE AT OR-1 (bottom of file; new, not lifted): tape_from_klines() — the
rename/ts step of the twin's bars_4h with the IO left behind — and snapshot()
— the per-symbol quantities the Daily Oracle's TIDE TABLES print.

KNOWN HAZARD FOR LOW-PRICED SYMBOLS [scout, OR-1; reported, not fixed — fixing
it would move the bytes above]: log() rounds to 2 dp and _span_at reads the
ROUNDED "extreme" back, so MICRO containment is distorted on sub-dollar
tapes, and leash px (1 dp) is meaningless there. The MACRO Range fields are
full precision and unaffected — snapshot() reads those, never the events.
...
screenshots). Hypotheses route to APOLLO under G-7.
