...
- suite **334 passed / 1 skipped / 0 failed** · F-CONV **4/4** · F-ROT **7/7** · F-MB-1 **8/8** · MAILBOX 43 links at both refreshes — a true no-op at proof time, then `1 created, 1 pruned` on the publish tail as this session's own report entered the rolling newest-25 window and displaced the oldest member. **§6 of the report first asserted the no-op alone, with the false explanation that nothing this session touched was in either mailbox set; it is rewritten, and a SECOND PUBLISH is disclosed** on the M4 precedent — a known-false line in a filed report is a defect under §3.1. Caught by reading the publish's output against the document just filed [verified]
PENDING (operator):
