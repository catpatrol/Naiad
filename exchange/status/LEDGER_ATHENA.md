...
- GOOGLE DRIVE IS SYNCING ~/Naiad, FOUND NOT LOOKED FOR. `git add -A` aborted with `fatal: unable to stat '.tmp.driveupload/1915'`. /Users/luis/Naiad/.tmp.driveupload holds 7.8 GB across 952 entries in the REPO ROOT, untracked and unignored; sizes match the phase archives (242,926,299 B = tc1_2026-07-27.zip). This is EXACTLY the 2026-08-05 condition the .gitignore bulk blocks were written to prevent, at twice the size, and it contradicts DATA RESIDENCY v2. AND THE IDENTITY GATE CANNOT SEE IT: the v2 gate greps OneDrive / com~apple~CloudDocs / Mobile Documents, not Google Drive Desktop, so it passed cleanly on a cloud-synced repo at the top of this session. Added to .gitignore so git cannot sweep it; NOTHING DELETED — it is Drive's working state, not Naiad's [verified]
