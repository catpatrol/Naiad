...
**F-C2-6 — no consultation — is a property, not a promise.** The decision path is a separate module, `scripts/tierc2_rules.py`, and it is proved four ways: (i) an **AST import scan** shows its imports are `{engine.indicators, engine.s1, numpy, dataclasses}` — zero analytics; (ii) its **transitive import closure** over 239 modules contains no `analytics` member; (iii) `engine/` itself imports no analytics module (invariant I-B), so the closure **cannot reach a registry symbol by any path**; (iv) no tape column name appears anywhere in the decision path. *(The line-grep the amendment asks for is printed too — and it failed on its first run, matching this module's own prose about not importing analytics. That false positive is exactly why the AST scan is the test of record and the grep rides beside it.)*
...
| **F-C2-3** closed register | PASS | 17 constants printed with source; `ATR_LEN`, `FEE_BPS_SIDE`, `STOP_BUF_ATR` **re-read live** from `configs/naiad_v0.yaml` and matched; `engine/trading.py` re-read and confirmed to still carry `TRAIL_B = 0.5` and the 200-bar 1h lookback; toll **derived** as 2 × 5.0 = 10.0 bps, never typed. |
